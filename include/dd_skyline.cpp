...
//    }
	...
	// 清空map的东西
	for (auto it : map) {
		delete reinterpret_cast<vector<BIGNUM*>*>(it.second);
	}
	//  情况原有的vector的所有内容
...
point_y* read_point_y(char* filePath) {
	...
	y->values = (BIGNUM**)malloc(sizeof(BIGNUM*) * y->d);
	int idx = 0;
	// 读取文件的第二行
...
RESULT select_and_export_points(unordered_map<int, void*>* ret, point_y* y, char* resultPath) {
	...
	// 写入查询到的不被支配的点
	for (auto idd : non_dominated) {
		auto itt = ret->find(idd);
		auto vec = reinterpret_cast<vector<BIGNUM*>*>(itt->second);
		fprintf(file, "%d: ", idd);
		// 使用 lambda 遍历并打印每个元素
		// 使用范围基的 for 循环遍历
